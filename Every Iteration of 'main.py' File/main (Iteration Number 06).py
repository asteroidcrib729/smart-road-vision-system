...
def calculate_iou_and_area_ratio(boxA, boxB):
    ...
    # 2. Area Ratio Calculation (Small / Large)
    # This tells us if one object is significantly smaller than the other
...
def main():
    ...
    while cap.isOpened():
        ...
        # Expanded Active Zone (Give bikes more room at the bottom)
        zone_top = frame_h * 0.15     
        zone_bottom = frame_h * 0.90 # Relaxed from 0.85 to 0.90 

        # 4. YOLO Inference
        # Extremely low confidence to catch every possible bike pixel
        ...
        while raw_candidates:
            ...
            for c in raw_candidates:
                ...
                # --- SIZE PROTECTION PROTOCOL ---
                # 1. Are they different classes? (e.g. Car vs Bike)
                is_diff_class = (current["label"] != c["label"])
                
                # 2. Is one significantly smaller than the other? (Ratio < 0.40)
                # If 'c' is less than 40% the size of 'current', it CANNOT be a duplicate.
                # It must be a separate object (e.g. Bike in front of Van).
                is_scale_mismatch = (size_ratio < 0.40)

                if is_diff_class and is_scale_mismatch:
                    # KEEP IT. Do not apply NMS.
                    survivors.append(c)
                elif is_diff_class:
                     # Different class but similar size? Overlap limit 90%
                     if iou < 0.90: survivors.append(c)
                else:
                    # Same class (Car vs Car). Strict NMS.
        ...
        for i in range(len(track_ids)):
            ...
            for j in range(i + 1, len(track_ids)):
                ...
                # PROTECTION: If scale mismatch, NEVER merge them.
                if size_ratio < 0.40:
                    continue 

                # Otherwise, merge if overlapping heavily
        ...
        for track_id, track in tracker.active_tracks.items():
            ...
            # Relaxed Stale Check: Draw even if stale for 4 frames (helps flickery bikes)
            if (tracker.frame_index - track.last_seen_frame) > 4: continue

            # Relaxed New Check: Draw new bikes immediately (no wait)
            is_bike = (track.state.vehicle_class == "bike")
            if not is_bike and (track.last_seen_frame - track.first_seen_frame) < 3: continue
        ...
        cv2.imshow("UVTP Scale-Aware", frame)
