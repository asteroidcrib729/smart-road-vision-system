...
def main():
    ...
    while cap.isOpened():
        ...
        for track_id, track in tracker.active_tracks.items():
            ...
            # --- NEW HIGH-RES BEST-SHOT REGISTRY LOGIC ---
            # 1. Map bounding box coordinates back to the original raw_frame dimensions
            orig_x1 = max(0, int(b.x1 / scale))
            orig_y1 = max(0, int(b.y1 / scale))
            orig_x2 = min(w, int(b.x2 / scale))
            orig_y2 = min(h, int(b.y2 / scale))

            # 2. Calculate current area to determine if this is a better shot
            current_area = (orig_x2 - orig_x1) * (orig_y2 - orig_y1)

            is_new_track = track_id not in vehicle_registry
            is_better_shot = not is_new_track and current_area > vehicle_registry[track_id].get("max_area", 0)

            if is_new_track or is_better_shot:
                # 3. Crop directly from the high-resolution raw_frame
                snapshot_img = raw_frame[orig_y1:orig_y2, orig_x1:orig_x2]
                ...
                if snapshot_img.size > 0:
                    if is_new_track:
                        ...
                        vehicle_registry[track_id]["max_area"] = current_area
                        # print(f"[UPDATED SNAPSHOT] {track_id} - Closer view captured")

                    # Overwrites the old file with the higher resolution version
