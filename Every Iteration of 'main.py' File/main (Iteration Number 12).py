                ...
                for track_id, track in tracker.active_tracks.items():
                    ...
                    if in_capture_zone:
                        ...
                        if is_new_track or is_better_shot:
                            # --- IMPROVEMENT 1: PADDING & 600p UPSCALING ---
                            # 1A. Add 15% padding around the bounding box to capture optical context
                            pad_w = int((orig_x2 - orig_x1) * 0.15)
                            pad_h = int((orig_y2 - orig_y1) * 0.15)

                            px1 = max(0, orig_x1 - pad_w)
                            py1 = max(0, orig_y1 - pad_h)
                            px2 = min(w, orig_x2 + pad_w)
                            py2 = min(h, orig_y2 + pad_h)
